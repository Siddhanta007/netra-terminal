...
interface WeaponBriefingProps {
  w: Weapon | null;
}

export default function WeaponBriefing({ w }: WeaponBriefingProps) {
  const { notes, setNotes, weaponLocked, setWeaponLocked } = useNetra();
  if (!w) return null;

  return (
    <div className="relative animate-in fade-in slide-in-from-top-4 duration-500 rounded-3xl overflow-hidden premium-shadow" style={{ background: 'var(--surface)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 relative z-10 p-8">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col h-full p-5 rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] group hover:border-[var(--accent)] transition-colors">
              ...
              <div className="text-[13px] leading-relaxed text-[var(--text-2)] font-medium font-sans flex-1">{w.logic}</div>
            </div>
            <div className="flex flex-col h-full p-5 rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] group hover:border-[var(--accent)] transition-colors">
              ...
              <div className="text-[13px] leading-relaxed text-[var(--text-1)] font-bold font-sans flex-1">{w.activation}</div>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex flex-col h-full p-5 rounded-2xl bg-[var(--surface-2)] border border-[var(--border)]">
        ...
        <div className="lg:col-span-5 flex flex-col pt-1">
          ...
          <div style={{ display: 'flex', gap: '8px', flexShrink: 0, marginTop: '8px' }}>
            ...
            <button onClick={() => setWeaponLocked(true)} className={`${weaponLocked ? 'btn-confirmed' : 'btn-confirm'} flex-1`} disabled={weaponLocked || !w.id}>
              {weaponLocked ? '✓ Armed' : 'Fire'}
            </button>
          </div>
        </div>
      </div>
