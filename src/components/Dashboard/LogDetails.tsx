...
export default function LogDetails({ log }: LogDetailsProps) {
  const {
    ...
    updateTradeLog, deleteTradeLog,
  } = useNetra();

  if (!log) return null;

  const entry = parseFloat(String(editFormData.entry_price)) || 0;
  const addCost = parseFloat(String(editFormData.additional_cost)) || 0;
  ...
  const getSelectionDisplay = (sel: unknown): string => {
    if (typeof sel === 'object' && sel !== null) {
      return Object.values(sel as Record<string, unknown>).filter(Boolean).join(', ');
    }
    return String(sel || '');
  };
    ...
    <div className="animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          <span className="text-[8px] font-bold uppercase tracking-widest text-gray-600 border border-gray-100 px-3 py-1 rounded-full bg-white">{String(log.phase1?.bias || '')}</span>
      ...
      <div className="space-y-4 pb-10">
        ...
        <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
            {[
              ...
              <div key={item.phase} className="flex flex-col items-center justify-center p-3 rounded-xl bg-gray-50/80 border border-gray-100 text-center transition-all hover:bg-white hover:shadow-md group">
                ...
                <div className="text-[11px] font-extrabold text-gray-900 leading-tight">{getSelectionDisplay(item.sel) || '—'}</div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="p-4 rounded-xl bg-blue-50/30 border border-blue-100/50 min-h-[100px]">
              <div className="text-[11px] text-gray-600 leading-relaxed font-medium">
                {[
                  ...
                  !log.phase1?.bias_note && !log.phase1?.auction_note && !log.phase1?.liquidity_note && !log.phase1?.behaviour_note
                    && 'No specific phase annotations recorded. Tactical parameters synchronized.',
                ].filter(Boolean).map((note, idx) => (
                  <p key={idx} className={idx > 0 ? 'mt-2 pt-2 border-t border-blue-100/30' : ''}>{note as string}</p>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={String(editFormData.trading_asset || '')} onChange={e => setEditFormData({ ...editFormData, trading_asset: e.target.value })} className="field w-full text-xs" placeholder="e.g. NIFTY 25000 CE" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2 block">Entry Price</label>
                <input type="number" value={String(editFormData.entry_price || '')} onChange={e => setEditFormData({ ...editFormData, entry_price: e.target.value })} className="field w-full text-xs" placeholder="0.00" />
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2 block">Additional Cost</label>
                <input type="number" value={String(editFormData.additional_cost || '')} onChange={e => setEditFormData({ ...editFormData, additional_cost: e.target.value })} className="field w-full text-xs" placeholder="Brokerage, slippage…" />
              </div>
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" value={String(editFormData.stop_loss || '')} onChange={e => setEditFormData({ ...editFormData, stop_loss: e.target.value })} className="field w-full text-xs" placeholder="0.00" />
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2 block">Exit Level</label>
                <input type="number" value={String(editFormData.exit_price || '')} onChange={e => setEditFormData({ ...editFormData, exit_price: e.target.value })} className="field w-full text-xs" placeholder="0.00" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[var(--surface)] p-4 rounded-xl border border-[var(--border)] shadow-sm">
          ...
          <div>
            ...
            <textarea value={String(editFormData.notes || '')} onChange={e => setEditFormData({ ...editFormData, notes: e.target.value })} className="field w-full text-xs py-3 resize-none" style={{ height: '80px' }} placeholder="Post-trade narrative, learnings, observations…" />
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="star-rating">
              {[1, 2, 3, 4, 5].map(n => (
                <button
                  key={n}
                  className={(editFormData.execution_rating as number) >= n ? 'active' : ''}
